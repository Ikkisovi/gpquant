"""
Quick demo of the three-line strategy

This is a simplified version for testing the system quickly.
It trains on a single symbol with a small population for faster execution.
"""
import pandas as pd
import numpy as np
...
def quick_demo():
    """Quick demonstration of the three-line strategy"""
    print("="*80)
    print("QUICK DEMO: Three-Line Strategy")
    print("="*80)
    ...
    train_size = min(200, int(len(market_df) * 0.7))
    train_df = market_df.iloc[:train_size].copy()
    test_df = market_df.iloc[train_size:train_size+100].copy()

    print(f"   Train size: {len(train_df)}, Test size: {len(test_df)}")

    # Calculate benchmark
    print("\n3. Calculating benchmark...")
    benchmark_close = calculate_equal_weight_benchmark(df, symbols[:5])  # Use first 5 symbols
    ...
    # Initialize with small population for quick demo
    print("\n5. Initializing genetic programming (small population for demo)...")
    sr = SymbolicRegressor(
        population_size=100,  # Small for demo
        tournament_size=10,
